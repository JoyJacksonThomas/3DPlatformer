...
namespace Rewired.Demos {
    ...
    public class DualShock4SpecialFeaturesExample : MonoBehaviour {
        ...
        private void Start()
        {
            stickpos = new Vector3[2];
            stickpos[0] = sticks[0].transform.localPosition;
            stickpos[1] = sticks[1].transform.localPosition;
        }

        private void Update() {
            if (!ReInput.isReady) return;

            sticks[0].transform.localPosition = stickpos[0]+ (Vector3.forward * player.GetAxisRaw("MoveVertical")) + (Vector3.right * player.GetAxisRaw("MoveHorizontal"));
